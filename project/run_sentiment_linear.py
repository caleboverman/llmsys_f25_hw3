...
from minitorch import SimpleOps, tensor
...
def cross_entropy_loss(out, y):
    ...
    log_out = minitorch.nn.logsoftmax(out, 0)
    log_one_out = minitorch.nn.logsoftmax(ones - out, 0)
    binary_cross_entropy_loss = (-(y * log_out + (ones - y) * log_one_out)).mean()
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        weights_out = self.weights.value.view(in_size, self.out_size)
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        avg_embedding = avg_embedding.view(avg_embedding.shape[0], avg_embedding.shape[2])
        layer_one_out = self.linear_one.forward(avg_embedding)
        relu_out = layer_one_out.relu()
        dropout_out = minitorch.dropout(relu_out, rate=self.dropout_prob)
        layer_two_out = self.linear_two.forward(dropout_out)
        output = layer_two_out.sigmoid().view(layer_two_out.shape[0])
        return output
        # END ASSIGN1_2
